---
lang: zh-cn
description: C#, Java, Python 和 Swift 软件开发团队。<br/>目前专注于Minecraft插件以及相关软件的开发。
resume_title: iXOR Technology
name: iXOR Technology
avatar: https://avatars.githubusercontent.com/u/74606353?s=200&v=4
contact:
  - icon: fab fa-github-square
    text: iXORTech
    url: https://github.com/iXORTech
  - icon: fas fa-envelope
    text: contact@ixor.tech
    url: mailto:contact@ixor.tech
---

{% raw %}
<center>
<a href='/'>English</a> | <a href='/zh-cn/'>简体中文</a>
</center>
{% endraw %}



## <i class="fas fa-project-diagram"></i> 项目

### MCDRemote Series

#### [MCDReforged-Remote](https://github.com/iXORTech/MCDReforged-Remote)

一个包含Flask框架的MCDR插件，允许用户通过相关API远程控制Minecraft服务器或实现双向聊天机器人。

#### [MCDRemoteBot](https://github.com/iXORTech/MCDRemoteBot)

实现了 MCDReforged-Remote API 的QQ聊天机器人，允许用户在QQ群内与服务器内玩家聊天或管理服务器。

#### [MCDRemote-Client](https://github.com/iXORTech/MCDRemote-Client)

MCDReforged-Remote 的跨平台桌面应用，允许用户远程控制服务器。

## <i class="far fa-id-badge"></i> 我们的成员

{% raw %}
 <btns rounded>
 <a href='https://github.com/Cubik65536' style="width: 320px">
   <img src='https://avatars.githubusercontent.com/u/72877496?v=4'>
   <b>Cubik65536</b>
   <span>创始人，项目经理<br/>Java & Python 工程师</span>
 </a>
 <a href='https://github.com/KevinZonda' style="width: 320px">
   <img src='https://avatars.githubusercontent.com/u/33132228?v=4'>
   <b>KevinZonda</b>
   <span>联合创始人, 首席软件工程师<br/>C# 工程师</span>
 </a>
 </btns>
 {% endraw %}

 ### Join us

如果你想加入我们，请向 [contact@ixor.tech](mailto:contact@ixor.tech) 发送邮件。 <br/>发挥想象力，让我们一起做一些让你感到自豪或对你有意义的事情。

